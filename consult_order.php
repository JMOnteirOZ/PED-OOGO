                ...
                <div class="container">
                    ...
                    <div id="masonrygold">
                        ...
                        <?php
                            ...
                            if (isset($_SESSION['uname'])) {
                                $sql = $db->prepare("SELECT * FROM `order`, user, category WHERE user.userID = `order`.userID AND category.categoryID = `order`.categoryID AND `order`.userID <> ? AND `order`.orderState = 'destaque' AND `order`.orderDateClose > now() AND `order`.orderID NOT IN (SELECT orderID FROM bannedorder)");
                                $sql->execute([$_SESSION['uid']]);
                            } else {
                                $sql = $db->prepare("SELECT * FROM `order`, user, category WHERE user.userID = `order`.userID AND category.categoryID = `order`.categoryID AND `order`.orderState = 'destaque' AND `order`.orderDateClose > now() AND `order`.orderID NOT IN (SELECT orderID FROM bannedorder)");
                                $sql->execute();
                            }
                            $rs = $sql->fetchAll();
                                ...
                                echo '<div class="column">';
                                    echo '<div class="blog-column">';
                                        echo '<div class="column-info">';
                                            echo '<h4 class="uppercase"><a href="#" class="gold">'.$row['orderTitle'].'</a></h4>';                     
                                            ...
                                            if (isset($_SESSION['uname'])) {
                                                echo '<a class="btn btn-primary btn-gold btn-xs" href="single-order.php?orderID='.$row['orderID'].'">Abrir Order</a>';
                                            }
                        ...
                        <?php
                            ...
                            foreach($rs as $row) {
                                ...
                                echo '<div class="column">';
                                    echo '<div class="blog-column">';
                                        echo '<div class="column-info">';
                                            echo '<h4 class="uppercase"><a href="#">'.$row['orderTitle'].'</a></h4>';                     
                                            echo '<ul class="blog-post-info">';
                                                echo '<li><a href="#"><i class="ion-person"></i> '.$row['userName'].'</a></li>';
                                                echo '<li><a href="#"><i class="ion-clock"></i> Faltam '.format_interval($diff).'</a></li>';
                                                echo '<li><a href="#"><i class="ion-social-euro"></i> '.$row['orderPrice'].'</a></li>';
                                            ...
                                            echo '<p>'.$row['orderDescription'].'</p>';
                                            if (isset($_SESSION['uname'])) {
                                                echo '<a class="btn btn-primary btn-xs" href="single-order.php?orderID='.$row['orderID'].'">Abrir Order</a>';
                                            }
                                            echo '<div class="blog-post-date">';
                                                echo '<div>';
                                                    echo '<img src="assets/image/'.$row['categoryName'].'.png" class="category">';
